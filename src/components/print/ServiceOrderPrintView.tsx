...
export default function ServiceOrderPrintView({ order }: { order: ServiceOrder }) {
  ...
  const diagnosticGroups = [
    ...
    { key: 'not_done', title: 'Itens não realizados' }
  ];
  const technicalHistory = deriveOrderTechnicalHistory(order);
  const statusLabel =
    order.status === 'pending' ? 'Recepção / Pendente' :
      order.status === 'in_progress' ? 'Em diagnóstico / Execução' :
        'Finalizada';
    ...
    <article className="print-doc">
      <header className="print-doc-header">
        <div>
          ...
          <div className="print-doc-subtitle">Gestao de oficina e serviços automotivos</div>
        ...
        <div className="print-doc-id-box">
          <div className="print-doc-type">Ordem de Serviço</div>
      ...
      <section className="print-doc-grid print-doc-section">
        <div>
          <h3>Cliente</h3>
          <p className="print-doc-strong">{order.customer_name || 'Não informado'}</p>
          <p>Telefone: {order.customer_phone || 'Não informado'}</p>
        </div>
        <div>
          <h3>Veículo</h3>
          <p className="print-doc-strong">{order.vehicle_model || 'Não informado'}</p>
          <p>Placa: {order.plate || 'Não informado'}</p>
        </div>
        <div>
          <h3>Entrada</h3>
        ...
        <div>
          <h3>Responsáveis</h3>
          <p>{order.technician_names?.length ? order.technician_names.join(', ') : 'Não atribuído'}</p>
        </div>
      </section>

      <section className="print-doc-section">
        <h2>Queixa / Serviço solicitado</h2>
        <p className="print-doc-box">{order.description || 'Não informado'}</p>
      </section>

      {technicalSymptoms.length > 0 && (
        <section className="print-doc-section">
          <h2>Sintomas técnicos constatados</h2>
      ...
      <section className="print-doc-section">
        ...
        <div className="print-doc-checklist">
          <div>
            ...
            <strong>{checklist.fuel_level === 'not_checked' ? 'Não verificado' : checklist.fuel_level}</strong>
          </div>
      ...
      {order.tests && order.tests.length > 0 && (
        <section className="print-doc-section">
          <h2>Diagnósticos guiados e testes</h2>
      ...
      {technicalHistory.length > 0 && (
        <section className="print-doc-section">
          <h2>Histórico técnico automático</h2>
      ...
      <section className="print-doc-section">
        ...
        <table className="print-doc-table">
          ...
          <tbody>{renderItems(serviceItems, 'Nenhum serviço informado.')}</tbody>
        </table>
      </section>

      <section className="print-doc-section">
        <h2>Peças / Itens aplicados</h2>
        <table className="print-doc-table">
          <thead>
            <tr>
              <th>Descricao</th>
      ...
      <section className="print-doc-total-row">
        <div>Subtotal serviços: <strong>{formatMoney(servicesTotal)}</strong></div>
      ...
      <section className="print-doc-terms">
        Autorizo a avaliação/execução dos serviços descritos nesta ordem, ciente dos itens de vistoria registrados no momento da entrada do veículo.
      </section>
